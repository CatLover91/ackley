...
Gene.prototype.cross = function(gene) {
    var swappoint = math.floor(math.random() * this.gstring.length / 2);
    var child = [];
    var c1 = {};
    var c2 = {};
    for (var i = 0; i < swappoint; i++) {
        ...
        c2[this.gstring[i]] = true;
        c1[gene[i]] = true;
    }
    for (i = swappoint; i < gene.length; i++) {
        if(this.gstring[i] in c2){
            child.push(-1);
            c2[-1] = true;
        } else {
            child.push(this.gstring[i]);
    ...
    var temp = 0;
    if(-1 in c1){
        for(i = 0; i < this.gstring.length; i++) {
            if(this.gstring[i] === -1){
                for(var q = 0; q < this.gstring.length; q++){
                    if(!(q in c1)) {
                        temp = q;
                    }
                }
